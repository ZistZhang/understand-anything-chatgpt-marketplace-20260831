...
description: Analyze Git diffs, working-tree changes, commits, or pull requests against
  an existing Understand Anything knowledge graph to identify changed nodes, affected
  components, architectural blast radius, and risks. Use for questions such as what
  changed, what might break, what this PR affects, or whether a change crosses architecture
  boundaries. Use `understand` first if the graph is missing or stale enough to make
