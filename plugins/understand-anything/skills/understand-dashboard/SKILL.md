...
description: Launch or prepare the interactive Understand Anything dashboard for an
  existing codebase, domain, knowledge, or design graph. Use when the user asks to
  view, browse, visualize, open, or explore the persistent graph interactively. Use
  `understand` first when the project has no usable knowledge graph.
---
