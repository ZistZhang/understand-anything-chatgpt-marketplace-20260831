---
name: understand
description: Build or refresh the persistent Understand Anything knowledge graph for
  a codebase. Use when the user asks to analyze, map, index, or understand an unfamiliar
  repository, create the project architecture graph, refresh a stale graph, or prepare
  the project for later chat, explain, diff, onboard, domain, or dashboard workflows.
  Prefer a more specific Understand Anything skill when an up-to-date graph already
  exists and the request is only to query, explain, compare changes, onboard, or visualize.
---
