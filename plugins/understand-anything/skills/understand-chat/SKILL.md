...
description: Answer broad questions about a codebase using an existing Understand
  Anything knowledge graph. Use for architecture, relationships, responsibilities,
  flows, dependencies, or how-the-system-works questions when `.ua/knowledge-graph.json`
  already exists. Prefer `understand-explain` for a deep dive into one specific file,
  function, class, or module; use `understand` first when no usable graph exists.
---
