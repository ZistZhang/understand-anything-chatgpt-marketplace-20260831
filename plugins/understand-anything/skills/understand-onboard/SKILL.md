...
description: Generate a structured onboarding guide for a project from an existing
  Understand Anything knowledge graph. Use when a user is new to a repository or team
  and asks where to start, what to read first, how the architecture is organized,
  what concepts matter, or for a guided learning path through the codebase. Use `understand`
  first when no usable graph exists.
---
