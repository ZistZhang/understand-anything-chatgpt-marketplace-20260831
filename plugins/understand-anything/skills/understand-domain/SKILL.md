...
description: Extract and visualize business domains, business flows, and process steps
  from a codebase. Use when the user wants the business or product view of what the
  software does rather than only file/class architecture. Prefer deriving from an
  existing Understand Anything graph when available; otherwise use the skill standalone
  with its lightweight scan or full mode.
---
